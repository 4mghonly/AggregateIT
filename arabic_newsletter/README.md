...
The Arabic briefing uses a **three-page, image-free dark command-brief design**. Page 1 contains expanded UAE coverage and three substantial lead stories. Page 2 contains secondary stories that did not appear on page 1, plus an executive assessment, developing storylines, concrete watch indicators and a compact regional coverage strip. Page 3 is a continuity board covering geographic regions, armed/non-state actors, conflicts and maritime tensions. Page 3 may carry forward the top ongoing story from the previous validated edition when it remains the most consequential item for that lens; it must not use empty decorative boxes or fabricate freshness. Do not repeat the same story in multiple news boxes on Pages 1–2. Maintain the restrained dark palette, Arabic font/RAQM shaping and encoding hygiene.
